...
- Setup step: a blank text editor where the analyst drafts the system prompt in their own words — no preset toggles, nothing prewritten. This text is appended to a fixed hidden baseline prompt (see Key Decisions) to form the full system prompt sent with every LLM call.
...
- Single-step undo per row, stored in SQLite: each row retains its current value and the immediately preceding value only. Analyst can undo the most recent applied change; undoing again with no prior value stored is a no-op with a clear message.
...
- Single chart per session, no auth; persistence is limited to the current chart's state and single-step undo data (see Key Decisions) — no multi-chart save/resume across sessions
...
4. **Single-step undo, not full revision history.** Rather than retaining every prior version of a row, each row keeps only its current value and the immediately preceding one, persisted in SQLite. This preserves the core safety-net property — no edit is unrecoverably lost the moment after it's applied — without building a multi-version history UI, which is more than the assignment's scope requires.
...
- Given the setup step, the analyst can draft a freeform system prompt before entering the chat, with no prewritten/preset content
...
- Given a row with an applied change, the analyst can undo it and the row reverts to its immediately prior value
- Given a row that was just undone (or never changed), a further undo attempt is a no-op with a clear message
...
- **Frontend:** React app — upload UI, setup step (freeform system prompt editor), split-pane chart (left) + chat (right) with a settings button for reopening/editing the system prompt mid-session, inline pending-proposal controls on chart cells
- **LLM:** used for (a) initial confidence-tier classification per row on upload, (b) opening chat message flagging weak rows, (c) chat-based refinement proposals, (d) row-reference disambiguation clarifying questions, (e) re-grounded evidence corrections; every call sends the fixed hidden baseline prompt plus the analyst's current freeform system prompt text
- **State:** SQLite database storing the current chart, its rows, and — per row — the current value plus one previous value for single-step undo; no auth, no multi-chart management
